...
#' Load positive or negative training images and get features.
#' 
#' @param directory_path The path to the training directory of the
#'        positive or negative set.
#' @param top_trim The number of pixels that should be trimmed from the top.
#' @param bottom_trim The number of pixels that should be trimmed from the bottom.
#' @return The data frame of features associated with the training positive
#'         or negative set.
get_train_features <- function(directory_path, top_trim, bottom_trim) {
  files <- list.files(
...
get_input_output <- function(pos_directory_path, neg_directory_path,
  ...
  inputs_pos <- get_train_features(pos_directory_path, top_trim, bottom_trim)
  inputs_neg <- get_train_features(neg_directory_path, top_trim, bottom_trim)
